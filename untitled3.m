% Название модели
modelName = 'TubeInTubeHeatExchanger';
...
% Теплопередача из теплоаккумулятора в теплообменник (через внешнюю стенку)
...
set_param([modelName, '/Gain_phase'], 'Gain', 'h_outer');
...
% Теплопередача из теплообменника в воздух (через внутреннюю стенку)
add_block('simulink/Math Operations/Product', [modelName, '/Q_air']);
add_block('simulink/Math Operations/Sum', [modelName, '/Sum_air']);
add_block('simulink/Math Operations/Gain', [modelName, '/Gain_inner']);
set_param([modelName, '/Gain_inner'], 'Gain', 'h_inner');
...
add_line(modelName, 'Delta_T_air/1', 'Gain_inner/1');
add_line(modelName, 'Gain_inner/1', 'Q_air/1');
